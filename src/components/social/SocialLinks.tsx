    ...
    <div className={`flex items-center gap-3 ${className}`}>
      ...
      {activePlatforms.map((platform) => {
        ...
        const url = platform.name === 'WhatsApp'
          ? `https://wa.me/${platform.handle?.replace(/[^0-9]/g, '')}`
          : getSocialProfileUrl(platform.handle!, platform.name.toLowerCase() as any);
        ...
        return (
          <a
            ...
            className={`flex items-center justify-center ${sizeClasses[size]} ${platform.bgClass} text-white rounded-full ${platform.hoverClass} transition-all`}
            title={platform.name}
